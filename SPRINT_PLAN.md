...
| 1.4 | Function App: embed with Azure OpenAI (ada-002) + cosine similarity retrieval | 5 | B | 1.3, 1.2 |
| 1.5 | Storage: Blob upload endpoint (`POST /uploads/`, CSV/JSON/TXT, 50 MB limit, content-type detection) | 3 | C | — |
| 1.6 | Storage: queue client with base64 encoding — enqueue jobs on experiment creation | 3 | C | — |
| 1.7 | Infra: GitHub Actions deploy workflows (backend Kudu zip + frontend OneDeploy + Function zip) | 5 | C | — |
| 1.8 | Backend: OpenAI client factory + chat endpoint (`POST /chat/`, 512 token limit) | 3 | D | — |
| 1.9 | Infra: App Service IP restrictions + Cosmos DB firewall (outbound IPs) | 2 | D | — |
| 1.10 | Infra: frontend `.env.production` with `VITE_API_URL`, verify build works with prod backend | 2 | D | 1.7 |
| 1.11 | Frontend: connect Upload + Results pages to real API (vibecoded) | 3 | any | 1.1, 1.5 |

**Totals**: A=10 · B=8 · C=11 · D=7
...
| # | Task | SP | Owner | Depends on | Status |
|---|------|----|-------|------------|--------|
| 2.1 | Backend: experiment status + progress polling endpoint (% complete per model) | 3 | A | 1.2 | |
| 2.2 | Backend: re-trigger experiment (reset status, clear old results, re-enqueue) | 2 | A | 1.8 | |
| 2.3 | Backend: constraint selection on upload — user picks task type + model size/cost filters | 3 | A | 1.2 | |
| 2.4 | Backend: summary/recommendation endpoint — rank models by score within user's constraints | 3 | A | 2.9 | |
| 2.5 | Function: add open-source models via fastembed (ONNX Runtime) — MiniLM, BGE | 5 | B | 1.6 | |
| 2.6 | Function: LLM-as-judge — per-document relevance scoring (0–10 scale, temp=0, seed=42, JSON output) | 5 | B | 1.6 | |
| 2.7 | Function: compute metrics per model (relevance score, latency, cost estimate) → write to Cosmos | 3 | B | 2.6 | |
| 2.8 | Function: add text-embedding-3-large as second Azure OpenAI model | 2 | D | 1.6 | |
| 2.9 | Function: progress tracking — update Cosmos per-model as benchmark runs | 3 | C | 2.7 | |
| 2.10 | Storage: poison queue handling — dead-letter detection, re-enqueue utility | 3 | C | 1.8 | |
| 2.11 | Infra: Function App deploy with bundled `.python_packages` (Docker-based pip install) | 5 | C | 2.5 | |
| 2.12 | Backend: input validation hardening (file types, size limits, structured error responses) | 3 | D | 1.7 | |
| 2.13 | Testing: integration tests for backend API (upload, experiments, queue, health) | 5 | D | 2.1 | |
| 2.14 | Infra: App Insights dashboard — query logs, set up failure alerts for function | 3 | D | — | |
| 2.15 | Frontend: dashboard, results charts, progress bar, sample datasets (vibecoded) | 5 | any | 2.1, 2.7 | |

**Totals**: A=11 · B=13 · C=11 · D=13
...
| Member | Sprint 1 (remaining) | Sprint 2 | Sprint 3 | Total remaining |
|--------|----------------------|----------|----------|-----------------|
| A | 10 SP | 11 SP | 10 SP | **31 SP** |
| B | 8 SP | 13 SP | 11 SP | **32 SP** |
| C | 11 SP | 11 SP | 8 SP | **30 SP** |
| D | 7 SP | 13 SP | 12 SP | **32 SP** |
...
| B | Function scaffold, ada-002 embedding | Open-source models, LLM judge, metrics | Retry logic, cost model, tests |
| C | Blob upload, queue, deploy workflows | Progress tracking, poison queue, fn deploy | Deploy script, security, arch diagram |
| D | Chat endpoint, firewall, frontend env | text-embedding-3-large, validation, integration tests, monitoring | E2E testing, presentation |
